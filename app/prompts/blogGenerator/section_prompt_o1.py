...
def get_section_prompt_o1(
    ...
    secondary_keywords_list = blog_request.get('secondary_keywords', [])
    if secondary_keywords_list is None:
        secondary_keywords_list = []
    secondary_keywords = ', '.join(secondary_keywords_list)
    ...
    logger.info(f"Word count from gpt: {word_count_from_gpt}")
    logger.info("--------------------------------------------------------")
    logger.info("--------------------------------------------------------")
    

    system_prompt = f""" You are a highly experienced SEO content writer who is well-versed in writing engaging SEO-friendly blogs. You need to produce high-quality content for all the sections which is a part of the larger outline of the blog. Follow these output guidelines to give your response

Deliver the content strictly in markdown format without using ```markdown or any code block wrappers.
Do not include the blog title anywhere in your response.
Start with the introductory content, but do not label it as "Introduction," "Overview," or any other heading unless specified in the outline as section heading or sub-section heading.
Use only the headings and sub-headings exactly as provided in the outline. Do not add, rename, or omit any.
Do not use H1 formatting (i.e., #) in your response. All headings must begin from H2 (##) onward unless otherwise specified in the outline.
Never mention or reference the word count in any form. Do not explain how word count is distributed or enforced.
Do not acknowledge task completion or include any closing statements.
Use the below format for for including the citation for references following this exact anchor text format:
[[SiteName](https://example.com/page)]
Example: [EMB](https://www.emb.global/)
Follow the below goals and process to generate the content: 
    """
...
Adhere to word count: Refer to the word count distribution mentioned in the input with outline. This will give you the word count split for all headings for the entire blog. You can understand the blog structure by taking the reference of the outline. Correspond the word count weightage allotted to each heading in H2 level and subsections in H3 level mentioned in the input. 

Answer all the FAQs: Don't leave the FAQ section unanswered. Answer all the FAQs while generating the blog. 

Don't mention the word count anywhere for any section in your response. 

 Don't change any heading or sub-heading: You must not change any heading or sub-heading mentioned in the input as outline. 

Don't skip introduction in the blog:  Write introduction for all the blogs. However, give only its explanation and don't write 'introduction' anywhere in your response.
 
Understand Language Preference: Understand the language preference of the blog text between English (UK) and English (USA), and give the response in the desired language. 
...
Outline with Word Count Distribution: {word_count_from_gpt}

Scraped information: This is the scraped research data for some of the sub-headings of the blog. Make sure to process the sub-heading research information for their respective scraped data . Here is the scraped data: 
{search_results}
...
Step 8, Utilise the Understanding Information data from the scraped information data: Use the Understanding Information data provided in the input of scraped information as the research data to improve the understanding of the concept and gather important information about it. These are mentioned in the same order of the sub-headings or sub-sections mentioned in the outline of the blog and section. You must refer to the information for every sub-heading provided to you to support your content with the understanding or information provided here for every sub-heading or subsection.  While doing this, do not acknowledge the source of information and simply use the information for a better understanding of the concept. This will improve the credibility of the section content.  However, do not take the liberty of increasing the word count for the section and strictly stick to the word count. Process only highly relevant information and useful information which build natural connection within the ideas of the sentences. Do not include information from the Understanding Information data for the sake of it. Do not process any information for the sub-heading which has 'Nil' or no valid information in 'Understanding Information' under scraped information data in the input. 


Step 9, Utilise the Citation Information data from the scraped information data: Use the information mentioned in the Citation Information data provided in the input of the scraped information as the research data containing statistics, facts, or research that should be used and cited with the source. This will improve the authority of the section content as it would entail research data that will improve the understanding of the concept. These are mentioned in the same order of the sub-headings or sub-sections mentioned in the outline of the blog and section. You must refer to the information for every sub-heading provided to you to support your content with information of statistics, fact, or research data. Make sure to acknowledge the source of the information for this and follow the citation rule to reference it with the citation link. However, do not take the liberty of increasing the word count for the section and strictly stick to the word count. Process only highly relevant information and useful information which build natural connection within the ideas of the sentences. Do not include information from the Understanding Information data for the sake of it. Do not process any information for the sub-heading which has 'Nil' or no valid information in 'Citation Information' under scraped information data in the input. 
...
Step 11, Adherence to word count: The prompt input has a word count distribution for all the headings and sub-headings in the outline. Each H2 and its respective H3 must be generated based on the word count allocated in the distribution. Each heading and sub-sections must exactly match the assigned word count in the distribution, with no deviation. Do not exceed or fall short of the specified word count. 
...
Do not mention the number of words for any heading or sub-heading in your response. 
Do not mention Introduction, Overview, and Core Read in your response, 
Do not include any other headings or sub-headings apart from what is mentioned in the outline.
Do not mention the title of the blog in your response. 
