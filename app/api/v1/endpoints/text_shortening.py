...
from app.services.text_shortening_service import TextShorteningService
...
@router.post("/stream")
async def stream_text_shortening(
    request: Request,
    *,
    ...
    async def generate_streaming_response():
        ...
        try:
            ...
            with get_db_session() as db:
                ...
                service = TextShorteningService(db=db, user_id=current_user_id, project_id=project_id)
                
                # Stream the shortening process in REAL-TIME
                async for event_data in service.stream_text_shortening_workflow(
                ...
                # ✅ Usage tracking and billing is now handled by TextShorteningService
...
@router.get("/health")
def text_shortening_health() -> Dict[str, Any]:
    """Health check endpoint for text shortening service"""
    return {
        "service": "text_shortening_streaming",
        "status": "healthy",
