...
class InvoicePDFGenerator:
    ...
    def _build_invoice_info(self, invoice):
        ...
        if isinstance(invoice, dict):
            ...
            invoice_date = self.formatter.format_date(invoice.get('issue_date', None))
            total_amount = self.formatter.format_currency(
                invoice.get('total_amount', 0), 
                invoice.get('currency', 'USD')
            )
            
            # Client information from nested dict
            ...
            client_country = invoice.get('client_country', '')
            client_postal = invoice.get('client_postal_code', '')
            client_phone = self.formatter.format_phone(invoice.get('client_phone', ''))
            
            # Invoice details
            invoice_number = invoice.get('invoice_number', '001')
        else:
            # Object format (original code)
            invoice_date = self.formatter.format_date(getattr(invoice, 'issue_date', None))
            total_amount = self.formatter.format_currency(
                getattr(invoice, 'total', 0), 
                getattr(invoice, 'currency', 'USD')  # Changed default to USD
            )
            ...
            client_email = self.formatter.safe_get_attr(invoice, 'client_email', '')
        ...
        # Build client address - use email if address is empty
        client_location = f"{client_city}, {client_country} - {client_postal}".strip(' ,-')
        if not client_location.strip(' ,-'):
            client_location = client_email if client_email else client_address
        ...
        invoice_info_data = [
            ...
            [
                Paragraph(client_address, self.style_manager.get_style('table_cell')),
                Paragraph("<b>Reference</b>", self.style_manager.get_style('section_header')),
                ''
            ],
            [
                Paragraph(client_location, self.style_manager.get_style('table_cell')),
            ...
            [
                Paragraph("Credits Recharge" if isinstance(invoice, dict) else "Professional Services", self.style_manager.get_style('table_cell')),
                '',
                ''
            ]
        ]
    ...
    def _build_items_table(self, invoice):
        ...
        if isinstance(invoice, dict):
            ...
            if items:
                ...
                table_data.append([
                    Paragraph("<b>Credits Recharge</b><br/>Account credit recharge service", 
    ...
    def _build_totals_section(self, invoice):
        ...
        if isinstance(invoice, dict):
            ...
            subtotal = total  # For wallet recharge, usually no separate subtotal
            tax_rate = invoice.get('tax_rate', 0)  # Wallet recharge usually doesn't have tax
            tax_amount = invoice.get('tax_amount', 0)
            currency = invoice.get('currency', 'USD')
        else:
            # Object format (original code)
            subtotal = getattr(invoice, 'subtotal', 0) or 0
            tax_rate = getattr(invoice, 'tax_rate', 0) or 0  # Changed default from 18 to 0
            ...
            currency = getattr(invoice, 'currency', 'USD')  # Changed default from INR to USD
