...
"""
    ...
    @staticmethod
    def get_streaming_convert_to_table_prompt(
        text_chunk: str,
...
You are processing chunk {chunk_number} of {total_chunks} in a streaming convert to table workflow.
...
- Language: {language}
Goal:
1. Convert the “Text to be edited” to a table.
2. Preserve the primary keyword, if and wherever mentioned.
3. Do not add facts or figures from your end.

Process:

The process is of 4-phases detailed below-

Phase 1: Identify the Variables

Phase 1, Step 1: Decide the columns- These should be the attributes you want to compare e.g., Feature, Description, Benefit.


Phase 1, Step 2: Decide the rows- Each row should represent one discrete item e.g., a product feature, a process step.


Phase 2: Keep Columns Uniform

Phase 2, Step 1: Each cell in a column should contain the same type of information.


Phase 2, Step 2: Avoid mixing styles (don’t have one row say “Yes” and another give a 3-line paragraph in the same column).


Phase 3: Limit Columns for Readability

Phase 3, Step 1: For presentations/reports → max 3–5 columns.
Phase 3, Step 2: For detailed documentation → you can go wider but keep headers short.


Phase 4: Use Condensed Wording

Phase 4, Step 1: Tables are for scanning, so strip filler words.
Phase 4, Step 2: Use bullet-like phrasing inside cells if needed.

Use Only this proper Html tags for the table exmaple: <table>
  <tr>
    <th>Company</th>
    <th>Contact</th>
    <th>Country</th>
...
Phase 4, Step 1: Tables are for scanning, so strip filler words.
Phase 4, Step 2: Use bullet-like phrasing inside cells if needed.
Use Only this proper Html tags for the table generation
Here is exmaple: 
<table>
  <tr>
    <th>Company</th>
