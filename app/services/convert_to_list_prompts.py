...
"""
    ...
    @staticmethod
    def get_streaming_convert_to_list_prompt(
        text_chunk: str,
...
- Language: {language}
Goal:
1. Convert the “Text to be edited” to a list.
2. Preserve the primary keyword, if and wherever mentioned.
3. Do not add facts or figures from your end.
4. Preserve all formatting <p>, <H3>, <H2>, <H1> tags

Process:

The process is of 5-phases detailed below-

Phase 1: Identify the Listable Units

Phase 1, Step 1: One core idea per bullet – No bullet should contain more than one main point.
Phase 1, Step 2: Look for natural groupings: steps in a process, reasons, features, or examples.
Phase 1, Step 3: Remove transition words (“Firstly…”) from prose-they’re redundant in a list.

Phase 2: Choose the Right Type of List

Phase 2, Step 1: Bulleted list (•) → Best for unordered ideas of equal weight (features, benefits).
Phase 2, Step 2: Numbered list (1, 2, 3) → Best for sequences, rankings, or priorities.
Phase 2, Step 3: Lettered list (A, B, C) → Best for categories or sub-points under a main heading.

Phase 3: Parallelize the Structure

Phase 3, Step 1:Keep grammatical form consistent across bullets (all start with verbs, or all nouns).
Phase 3, Step 2: Use concise labels at the start of each point, then add detail after a colon.




Phase 4: Use Sub-Bullets for Depth

Phase 4, Step 1: If a point needs examples or clarifications, add a nested list instead of stuffing it into one bullet.

Phase 5: Keep it Visually Digestable

Phase 5, Step 1: Limit to 5–7 main bullets per list (human short-term memory sweet spot).
Phase 5, Step 2: For longer content, break into multiple lists under subheadings instead of one long scroll.
OUTPUT:
